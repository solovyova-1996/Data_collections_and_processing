...
vacancy_all_info_params = {'class':'vacancy-serp-item vacancy-serp-item_redesigned'}
...
vacancy_name = input('Введите название вакансии(название вакансии на латинице,например,слесарь-slesar,бухгалтер-bukhgalter,программист-programmist,стоматолог-stomatolog,учитель-uchitel): ')
# vacancy_name = translit(vacancy_name,'ru',reversed=True)
...
def handler_salary(salary_list):
    if 'от' in salary_list and len(salary_list) == 4:
        ...
        max_price = None
        currency = salary_list[3][:3] if len(salary_list[3]) == 4 else salary_list[3]
        ...
        currency = salary_list[5][:3] if len(salary_list[5]) == 4 else salary_list[5]
        return min_price, max_price, currency

def filling_dict(title,link,employer,vacancy_address,description,min_price,max_price,currency):
    return {
        'title':title,
        'link':link,
        'employer':employer,
...
while add_url:
    ...
    for vacancy in list_vacancy:
        ...
        try:
            ...
            employer = None
        try:
            vacancy_address = vacancy.find('div',vacancy_address_params).getText()
        except AttributeError:
        ...
        try:
           ...
           currency = None
            ...
            min_price = None
            currency = None
        vacancy_info_storage.append(filling_dict(title,link,employer,vacancy_address,description,min_price,max_price,currency))
